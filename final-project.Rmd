...
* The investigation uses a dataset of 1,000 Expedia user searches along with certain variables related to users or the top 3 recommended properties between June $1^{st}$, 2021 and July $31^{st}$, 2021.
...
* A confidence level 95% means that 95% of confidence intervals generated in a similar manner (i.e. resampling from the sample of 1,000 user data 3,000 times) will capture the true proportion of consumers between June $1^{st}$, 2021 and July $31^{st}$, 2021 who purchased a top 3 recommended property within 180 minutes.
...
* The calculated test statistic from the dataset for the difference between the mean number of clicks received within 180 minutes of a search for non-discounted and discounted top properties is `r round(test_stat, digits=4)`. 

* This means that within the sample, discounted top 3 properties get `r -round(test_stat, digits=4)` less clicks than non-discounted properties on average within 180 minutes of a user search. 
...
* Since the p-value is between 0.01 and 0.05, there is moderate evidence against the null hypothesis which states that the mean number of clicks for discounted and non-discounted properties received within 180 minutes of a user search is the same between between June $1^{st}$, 2021 and July $31^{st}$, 2021.
...
* For instance, if the correlation is found to be positive, larger properties which can fit more people may be booked for longer interval of time.
...
* However, the $R^2$ value of `r round(r_squared, 4)` for the linear regression model implies only `r round(r_squared, 4) * 100`% of variability in the length of trip is accounted by the number of adults on the trip.
...
* For research questions 1 and 2, the variables is_trans{n} and num_clicks{n} only measure user events like transaction or clicks for 180 minutes after a search. This time interval limitation carries over to the statistical analysis. 
...
* For research question 3, we intended to research how the number of adults, children and infants affect the length of travel. However, only 2 user searches had infants in the travel group. So, we removed the number of infants as a potential predictor. 
...
* The bounds of the interval are low which implies the recommendation algorithm has room for improvement in terms of increasing the proportion of users who make a transaction within 180 minutes of a user search.
...
* There is weak evidence against the null hypothesis that the mean number of clicks within 180 minutes of a user search for discounted and non-discounted properties is the same. 

* We recommend Expedia perform A/B testing with 2 variations of the website which differ by the number of discounted listed properties.$^{[2]}$
...
* It was found that as the number of adults increases, the length of travel increases. 
...
* However, the low $R^2$ value of `r round(r_squared, 4)` of the model implies there are other variables (apart from number of adults) that explain the variability in travel length. 

* To get a more "complete" model, Expedia may choose to explore how other variables (including those that do not appear in the current dataset) affect length of travel. 
