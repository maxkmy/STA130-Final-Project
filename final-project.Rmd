...
* We formulate 3 research questions that answer these "broad" questions:
...
**Research Question**: What is the proportion of consumers between June $1^{st}$, 2021 and July $31^{st}$, 2021 that purchase one of the top 3 recommended properties within 180 minutes of a search? 
...
expedia_data_q1 %>%  
  ...
  labs(x="Transaction made by users for any \n of top 3 recommended properties",
       y="Count", 
       title="Number of users who transacted \nor have not transacted a top 3 \nrecommended properties in 180 \nminutes of a search")
```

This figure shows that 9 out of 1,000 consumers in the sample purchase a top 3 listing recommended by Expedia within 180 minutes of their search.
...
**Hypothesis:** 
The population contains top 3 recommended properties between June $1^{st}$, 2021 and July $31^{st}$, 2021. 
...
* This may lead to the development new marketing strategies in terms of discounting more or less properties to maximize profit. 
...
* Expedia can consider the linear regression model when purchasing properties in bulk or advising hoteliers regarding the expected intervals of booking. For instance, when booking properties with many rooms to resell, Expedia should book these properties for longer period to reduce chances of them not being transacted. 

* However, the low $R^2$ value of `r round(r_squared, 4)` implies there are other variables that explain the variability in travel length. 
